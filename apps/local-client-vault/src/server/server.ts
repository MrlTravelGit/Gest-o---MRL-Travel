...
const server=createServer({cert:readFileSync(config.certPath),key:readFileSync(config.keyPath),minVersion:"TLSv1.2"},async(req,res)=>{
  Object.entries(securityHeaders).forEach(([key,value])=>res.setHeader(key,value));
  const origin=req.headers.origin;if(origin&&origin===config.mainAppOrigin){res.setHeader("Access-Control-Allow-Origin",origin);res.setHeader("Access-Control-Allow-Credentials","true");res.setHeader("Vary","Origin");}
  ...
  const url=new URL(req.url||"/",config.publicOrigin);if(url.pathname==="/health")return json(res,200,{status:"local-only",https:true});
  try{
    if(url.pathname==="/api/auth/login"&&req.method==="POST"){const input=await jsonBody(req);const result=await login(String(input.username||""),String(input.password||""),String(input.totp||""),address(req));res.setHeader("Set-Cookie",`vault_session=${result.token}; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=${Math.floor(config.sessionMaxMs/1000)}`);return json(res,200,{user:result.user,csrf:result.csrf,expiresAt:result.expiresAt});}
    ...
    if(url.pathname==="/api/auth/logout"&&req.method==="POST"){logout(session!.sessionId,session!.userId,address(req));res.setHeader("Set-Cookie","vault_session=; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=0");return json(res,200,{ok:true});}
...
server.listen(config.port,config.host,()=>{process.stdout.write(`MRL Vault ativo em ${config.publicOrigin}\n`);});
