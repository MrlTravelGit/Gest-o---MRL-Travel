...
import { getClient,isClientId,listClients,revealCredential,saveCredential,updatePersonal } from "./vault-service.js";
import { retryClientSync,syncClients } from "./sync-service.js";
...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(url.pathname==="/api/sync/clients/run"&&req.method==="POST"){requireRole(session!.role,["vault_admin"]);const result=await syncClients();audit({userId:session!.userId,action:"manual_client_sync",result:"success",localAddress:address(req),metadata:result});return json(res,200,result);}
    const retrySync=url.pathname.match(/^\/api\/clients\/([^/]+)\/retry-sync$/i);
    if(retrySync&&req.method==="POST"){requireRole(session!.role,["vault_admin","vault_operator"]);if(!isClientId(retrySync[1]))throw new Error("INVALID_CLIENT_ID");const result=await retryClientSync(retrySync[1]);audit({userId:session!.userId,action:"client_sync_retry",clientId:retrySync[1],result:"success",localAddress:address(req),metadata:result});return json(res,200,result);}
    ...
    if(reveal&&req.method==="POST"){requireRole(session!.role,["vault_admin","vault_operator"]);const input=await jsonBody(req);const purpose=input.purpose==="copy"?"copy":"reveal";return json(res,200,revealCredential(reveal[1],reveal[2],reveal[3] as "login"|"password",session!.userId,address(req),purpose),{"Cache-Control":"no-store, no-cache, must-revalidate","Pragma":"no-cache"});}
    ...
    if(documentRead&&req.method==="GET"){requireRole(session!.role,["vault_admin","vault_operator"]);const file=readDocument(documentRead[1],documentRead[2],session!.userId,address(req));const inline=url.searchParams.get("disposition")==="inline"&&/^(application\/pdf|image\/|text\/plain)/.test(file.mimeType);res.writeHead(200,{"Content-Type":file.mimeType,"Content-Disposition":`${inline?"inline":"attachment"}; filename=documento-protegido`,"Cache-Control":"no-store","X-Content-Type-Options":"nosn"});return res.end(file.bytes);}
  ...
  }catch(error){const message=error instanceof Error?error.message:"INTERNAL_ERROR";const status=message==="FORBIDDEN"?403:message==="NOT_FOUND"?404:message==="PAYLOAD_TOO_LARGE"?413:message.startsWith("SYNC_")?503:400;audit({action:"request_failed",result:"failed",localAddress:address(req),metadata:{code:message}});json(res,status,{error:message});}
});
