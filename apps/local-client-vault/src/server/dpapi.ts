...
const powershell = (operation: "Protect" | "Unprotect", input: Buffer) => {
  if (process.platform !== "win32") throw new Error("DPAPI exige Windows. Nao existe fallback de chave em texto simples.");
  const script = `$ErrorActionPreference='Stop';Add-Type -AssemblyName System.Security;$raw=[Convert]::FromBase64String([Console]::In.ReadToEnd());$scope=[Security.Cryptography.DataProtectionScope]::CurrentUser;$out=[Security.Cryptography.ProtectedData]::${operation}($raw,$null,$scope);[Console]::Out.Write([Convert]::ToBase64String($out))`;
