...
import { AlertTriangle, ArrowLeft, Coins, Gem, WalletCards } from "lucide-react";
...
export function AdminClientDetailPage() {
  ...
  return (
    <AppShell title={detail.data?.client.fullName ?? "Gestão do cliente"} subtitle="Pontos, custo médio, clubes e vencimentos">
      <div className="page-toolbar detail-toolbar"><Link className="secondary-button" to="/admin/clientes"><ArrowLeft size={17} /> Clientes</Link>{detail.data && <span className="status-pill">{detail.data.client.contractStatus ?? detail.data.client.status}</span>}</div>
