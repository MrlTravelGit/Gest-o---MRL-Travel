...
import { AlertTriangle, BadgeCheck, CalendarCheck, Gem } from "lucide-react";
...
export function AdminClubsPage() {
  ...
  const [form, setForm] = useState({
    clientId: "", accountId: "", planId: "", status: "active" as "active" | "paused" | "cancelled",
    startsOn: today, endsOn: "", expectedCreditDay: 1, nextCompetence: month, notes: "",
    effectivePrice: "", contractedOffer: "", selectedStreaming: "", joiningBonusEligible: false,
    eligibilityJoinedOn: today, eligibilityReason: "",
  });
  ...
  const selectedPlan = catalog.data?.plans.find((plan) => plan.planId === form.planId);
  const isStreaming = selectedPlan?.productFamily === "Clube Smiles + Streaming";
  const save = useMutation({
    mutationFn: () => upsertClubSubscription({ ...form, effectivePrice: form.effectivePrice ? parseMoneyPtBr(form.effectivePrice) : null }),
    onSuccess: () => {
      setForm((current) => ({ ...current, notes: "", eligibilityReason: "" }));
      void Promise.all([
  ...
  return <AppShell title="Clubes" hideHeading>
    <PageHeader eyebrow="Clubes de fidelidade" title="Catálogo e assinaturas" description="Planos, preços, bônus e benefícios são versionados. Pontuação mensal e bônus de adesão seguem fluxos separados." />
    ...
    {options.data && catalog.data && <section className="module-grid two-columns">
      <form className="module-form" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
        <div className="form-title"><Gem /><div><h2>Assinatura do cliente</h2><p>O backend confere cliente, conta, programa, versão e elegibilidade.</p></div></div>
        <div className="form-grid">
          ...
          <label>Plano<select value={form.planId} onChange={(event) => setForm((current) => ({ ...current, planId: event.target.value, joiningBonusEligible: false }))} required><option value="">Selecione</option>{availablePlans.map((plan) => <option key={plan.planId} value={plan.planId}>{plan.programName} · {plan.name} · v{plan.catalogVersion ?? 1}</option>)}</select></label>
          <label>Status<select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as typeof current.status }))}><option value="active">Ativo</option><option value="paused">Pausado</option><option value="cancelled">Cancelado</option></select></label>
          ...
          {isStreaming && <>
            <label>Preço efetivamente pago<input inputMode="decimal" value={form.effectivePrice} onChange={(event) => setForm((current) => ({ ...current, effectivePrice: event.target.value }))} required placeholder="R$ 75,90" /></label>
            <label>Oferta contratada<input value={form.contractedOffer} onChange={(event) => setForm((current) => ({ ...current, contractedOffer: event.target.value }))} required /></label>
            <label>Streaming<select value={form.selectedStreaming} onChange={(event) => setForm((current) => ({ ...current, selectedStreaming: event.target.value }))} required><option value="">Selecione</option><option>Disney+</option><option>HBO Max</option><option>Sky+</option><option>PK Streaming</option></select></label>
            <label className="binary-line"><input type="checkbox" checked={form.joiningBonusEligible} onChange={(event) => setForm((current) => ({ ...current, joiningBonusEligible: event.target.checked }))} /> Elegível ao bônus de adesão</label>
            {form.joiningBonusEligible && <><label>Data da adesão<input type="date" value={form.eligibilityJoinedOn} onChange={(event) => setForm((current) => ({ ...current, eligibilityJoinedOn: event.target.value }))} required /></label><label className="field-full">Confirmação da elegibilidade<textarea minLength={10} required value={form.eligibilityReason} onChange={(event) => setForm((current) => ({ ...current, eligibilityReason: event.target.value }))} placeholder="Oferta conferida no contrato do cliente…" /></label></>}
          </>}
        ...
        {isStreaming && <div className="catalog-contract-warning"><AlertTriangle /><div><strong>Bônus não recorrente</strong><span>As 14.500 milhas só serão registradas uma vez e apenas se a elegibilidade for confirmada.</span></div></div>}
        {save.isError && <div className="form-error">{save.error.message}</div>}
        {save.isSuccess && <div className="form-success">Assinatura salva; mensalidade e eventual bônus foram tratados separadamente.</div>}
      ...
      <div className="module-form read-panel">
        ...
        <div className="catalog-list">{catalog.data.plans.map((plan) => <article key={plan.planId}><strong className="program-inline"><LoyaltyProgramMark name={plan.programName} size="sm"/>{plan.programName} · {plan.name}</strong><span>{formatPoints(plan.monthlyPoints)} / mês {plan.pointsDoNotExpire ? "· sem expiração enquanto ativo" : ""}{plan.joiningBonusPoints ? ` · bônus de adesão ${formatPoints(plan.joiningBonusPoints)}` : ""}</span><small>v{plan.catalogVersion ?? 1} · {plan.priceQualifier === "from" ? "preço a partir de" : "preço fixo"} · {plan.sourceVerifiedOn} · {plan.sourceNotes ?? "Fonte oficial"}</small></article>)}</div>
      </div>
    </section>}
