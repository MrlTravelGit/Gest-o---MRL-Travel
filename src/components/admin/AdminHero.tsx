...
export function AdminHero({ overview }: { overview: AdminOverview }) {
  ...
  return <section className="admin-hero" aria-labelledby="admin-hero-title">
    ...
    <div className="admin-hero-content">
      <div className="hero-copy"><span className="hero-kicker">Central operacional · {roleLabel(overview.role)}</span><p className="hero-greeting">Olá, {overview.operatorName}.</p><h1 id="admin-hero-title">Gestão de <em>Milhas</em></h1><p>Decisões claras, saldos oficiais e cada operação rastreada em um único lugar.</p><div className="hero-actions">{overview.canArchive && <Link className="primary-button" to="/admin/clientes/novo"><Plus size={17} /> Cadastrar cliente</Link>}{overview.canWrite && <Link className="secondary-button" to="/admin/viagens-e-economia">Lançar operação <ArrowRight size={17} /></Link>}</div></div>
