...
import { shouldHideSavingInsteadOfDeleting } from "@/lib/savings-match-key";
import { calculateSavingsSummary, isSavingsDateRangeInvalid, isWithinSavingsDateRange } from "@/lib/savings-date-filter";
import { formatCurrency, formatDate } from "@/lib/formatters";
import { cancelTravelSaving, deleteTravelSaving, getAdminSavingEvidenceUrl, getClientCashback, getTravelSales, hideTravelSaving, previewTravelSavingVoid, recordCashbackRedemption, removeSavingEvidence, removeTravelSaleFromResult, updateClientCashbackConfig, updateTravelSaving, uploadSavingEvidence } from "@/services/travel-economy";
import type { CashbackSummary, ClientCashbackState, TravelSale, TravelSalesResult } from "@/types/admin-modules";
...
export function ClientSavingsPanel({ clientId, canWrite }: { clientId: string; clientName: string; canWrite: boolean }) {
  ...
  const dateRangeInvalid = isSavingsDateRangeInvalid(startDate, endDate);
  const query = useQuery({ queryKey: ["travel-sales", clientId, "all", startDate, endDate], queryFn: () => getTravelSales({ clientId, status: "all", startDate: startDate || undefined, endDate: endDate || undefined, limit: 100 }), enabled: !dateRangeInvalid });
  ...
  const visibleSavingsItems = useMemo(() => query.data?.items.filter((item) => item.deletedAt == null && !["deleted", "removed", "archived", "hidden"].includes(String(item.status).toLowerCase())) ?? [], [query.data]);
  const tabFilteredSavingsItems = useMemo(() => visibleSavingsItems.filter((item) => status === "all" || item.status === status), [visibleSavingsItems, status]);
  const dateFilteredSavingsItems = useMemo(() => tabFilteredSavingsItems.filter((item) => isWithinSavingsDateRange(item, startDate, endDate)), [tabFilteredSavingsItems, startDate, endDate]);
  const periodSummary = useMemo(() => calculateSavingsSummary(dateFilteredSavingsItems, cashback.data?.transactions ?? [], startDate, endDate), [dateFilteredSavingsItems, cashback.data?.transactions, startDate, endDate]);
  const periodTransactions = useMemo(() => cashback.data?.transactions.filter((item) => isWithinSavingsDateRange(item, startDate, endDate)) ?? [], [cashback.data?.transactions, startDate, endDate]);
  ...
  return <section className="module-form client-savings-panel">
    ...
    {cashback.data && <><div className="cashback-ledger-summary"><Summary label="Total economizado" value={formatCurrency(periodSummary.totalSaved)}/><Summary label="Cashback gerado" value={formatCurrency(periodSummary.cashbackGenerated)}/><Summary label="Saldo disponível" value={formatCurrency(periodSummary.availableBalance)} highlight/><Summary label="Utilizado / pago" value={`${formatCurrency(periodSummary.usedAmount)} / ${formatCurrency(periodSummary.paidAmount)}`}/></div><div className="client-savings-toolbar"><span className={`cashback-state ${cashback.data.config.enabled ? "enabled" : "disabled"}`}>{cashback.data.config.enabled ? "Cashback ativo" : "Cashback desabilitado"}{cashback.data.config.defaultPercentage ? ` · padrão ${cashback.data.config.defaultPercentage}%` : ""}</span><div><button className="secondary-button" disabled={!canWrite} onClick={() => setConfigOpen(true)}><Settings2 size={15}/> Configurar</button><button className="secondary-button" disabled={!canWrite || cashback.data.summary.available <= 0} onClick={() => setUseOpen(true)}><Banknote size={15}/> Registrar uso / pagamento</button><Link className="secondary-button" to={`/admin/viagens?clientId=${clientId}`}><Plus size={15}/> Nova economia</Link></div></div></>}
    <SavingsDateFilter startDate={startDate} endDate={endDate} onStartDateChange={setStartDate} onEndDateChange={setEndDate} idPrefix={`admin-savings-${clientId}`}/>
    ...
    {query.isLoading && <div className="panel-state">Carregando economias...</div>}{query.isError && <div className="form-error">{query.error.message}</div>}{!query.isLoading && !query.isError && dateFilteredSavingsItems.length === 0 && <div className="panel-state">{startDate || endDate ? "Nenhuma economia encontrada neste período." : "Nenhuma economia registrada."}</div>}
    <div className="client-savings-list">{dateFilteredSavingsItems.map((sale) => <article key={sale.id} className={sale.status==="voided" ? "saving-voided" : ""}>
    ...
    {deleting && (
      <DeleteSavingDialog sale={deleting} onClose={() => setDeleting(null)} onDeleted={async () => {
        setDeleting(null);
        setSuccessMessage("Economia removida. Lista e totais foram atualizados.");
        await refresh();
      }}/>
    )}
  </section>;
...
function DeleteSavingDialog({ sale, onClose, onDeleted }: { sale: TravelSale; onClose: () => void; onDeleted: () => Promise<unknown> }) {
  ...
  const hideInstead = shouldHideSavingInsteadOfDeleting(sale);
  const preview = useQuery({ queryKey: ["saving-delete-preview", sale.id], queryFn: () => previewTravelSavingVoid(sale.id), enabled: !hideInstead && sale.status === "active" });
  const mutation = useMutation({
    mutationFn: async (): Promise<{ summary: CashbackSummary }> => hideInstead
      ? hideTravelSaving(sale.id, reason, sale.updatedAt)
      : deleteTravelSaving(sale.id, reason, sale.updatedAt),
    onMutate: async () => {
      ...
      queryClient.setQueryData<ClientCashbackState>(["client-cashback", sale.clientId], (current) => current && sale.status === "active"
        ? { ...current, summary: { ...current.summary, generated: current.summary.generated - sale.cashbackAmount, available: current.summary.available - sale.cashbackAmount } }
        : current);
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Remover economia"><form className="confirm-modal void-saving-dialog" onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }}><span className="eyebrow">Remoção auditável</span><h2>{hideInstead ? "Ocultar economia" : "Excluir economia"}</h2><p>{hideInstead ? "Essa economia veio de um histórico migrado ou não editável. Ela será ocultada do painel e removida dos totais, mas permanecerá registrada para auditoria." : "O item sairá das listas, totais e painel público. O registro permanecerá no banco somente para auditoria."}</p>{preview.isLoading && <div className="panel-state">Calculando impacto financeiro...</div>}{preview.data?.blocked && <div className="form-error">A exclusão está bloqueada: {formatCurrency(preview.data.allocated)} de cashback já foi utilizado ou pago.</div>}{preview.isError && <div className="form-error">{preview.error.message}</div>}<label>Motivo da remoção<textarea minLength={5} required value={reason} onChange={(event) => setReason(event.target.value)}/></label>{mutation.isError && <div className="form-error">{mutation.error.message}</div>}<div className="dialog-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button className="danger-confirm-button" disabled={mutation.isPending || preview.isLoading || preview.data?.blocked || reason.trim().length<5}>{mutation.isPending ? "Removendo..." : "Confirmar remoção"}</button></div></form></div>;
}
