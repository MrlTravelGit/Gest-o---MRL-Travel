...
export function ClientInvoicesPanel({ clientId }: { clientId: string }) {
  ...
  return <section className="data-section client-invoices-panel">
    ...
    {items.length > 0 && <div className="responsive-table"><table><thead><tr><th>Competência</th><th>Banco / conta</th><th>Cartão</th><th>Valor</th><th>Previsão</th><th>Recebidos</th><th>Situação</th><th>Ação</th></tr></thead><tbody>{items.map((item) => <tr key={item.statementId}><td>{formatDate(item.statementMonth)}</td><td><strong>{item.institutionName || "Vínculo pendente"}</strong><small>{item.accountPersonType || "—"}</small></td><td>{item.cardLabel || <span className="invoice-pending-copy"><AlertTriangle /> Sem cartão</span>}</td><td>{formatCurrency(item.totalSpend)}</td><td>{item.predictedPoints == null ? "—" : formatPoints(item.predictedPoints)}</td><td>{formatPoints(item.receivedPoints)}</td><td><StatusBadge status={item.predictionStatus} /></td><td><Link className="table-action" to={`/admin/faturas?clientId=${clientId}&statementId=${item.statementId}`}><Pencil /> Editar</Link>{!item.cardId && <Link className="table-action" to={`/admin/faturas?clientId=${clientId}&statementId=${item.statementId}`}><CreditCard /> Associar</Link>}</td></tr>)}</tbody></table></div>}
  </section>;
}
