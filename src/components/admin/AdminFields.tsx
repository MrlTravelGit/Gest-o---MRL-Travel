...
export function StatusBadge({ status }: { status: string }) { const labels: Record<string, string> = { active: "Ativo", ended: "Arquivado", paused: "Pausado", lead: "Lead", open: "Aberto", quoting: "Em cotação", converted: "Convertido", cancelled: "Cancelado", cash: "Dinheiro", miles: "Milhas" }; return <span className={`status-badge status-${status}`}>{labels[status] ?? status}</span>; }
