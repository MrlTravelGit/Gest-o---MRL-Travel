...
- `DIAGNOSTICAR-COFRE.cmd`: verifica serviço, rede, HTTP, firewall, build, permissões e logs sem alterar dados.
...
- HTTP restrito à LAN, sem certificado, CA ou chave TLS. A chave mestra dos dados continua protegida pelo DPAPI e as pastas usam ACL restrita.
...
O painel nunca deve ser publicado na internet ou ter a porta encaminhada no roteador. Para instalar, copie esta pasta completa para `D:\Michael\Sistema Gestão - (Servidor Local)\local-client-vault` e execute `INICIAR-COFRE.cmd`.
