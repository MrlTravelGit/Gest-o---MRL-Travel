...
const server=createServer(async(req,res)=>{
  ...
  }catch(error){const raw=error instanceof Error?error.message:"INTERNAL_ERROR";const message=/FOREIGN KEY constraint failed/i.test(raw)?"INVALID_REFERENCE":raw;const status=message==="FORBIDDEN"?403:message==="NOT_FOUND"||message==="CLIENT_NOT_SYNCED"?404:message==="PAYLOAD_TOO_LARGE"?413:400;audit({action:"request_failed",result:"failed",localAddress:address(req),metadata:{code:message}});json(res,status,{error:message});}
});
