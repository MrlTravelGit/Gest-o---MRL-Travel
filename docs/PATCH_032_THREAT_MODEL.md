...
| Comprometimento da conta Windows do serviço | Acesso à chave protegida por DPAPI | Privilégio mínimo, logon interativo negado, MFA do cofre, rotação/revogação, Defender e auditoria fora do alcance de operadores | Administrador local comprometido continua sendo ameaça crítica |
| Usuário interno indevido | Revelação ou cópia não autorizada | RBAC, escopo por cliente, MFA, reautenticação para ações críticas, segredo sob demanda e auditoria sem conteúdo | Captura manual após revelação autorizada não pode ser eliminada totalmente |
