...
-- PATCH 060: expose the complete active catalog while keeping wallet totals
-- sourced exclusively from the same ledger/snapshot rules as the client panel.
...
begin
  ...
  with visible_accounts as (
    ...
    left join lateral (
      select coalesce(sum(pt.points_delta) filter (
    ...
    left join lateral (
      select
        min(el.expires_on) filter (where el.expires_on >= current_date) as next_expiration_date,
  ...
  program_rollup as (
    ...
    group by va.program_id
  )
  select
    lp.slug::text,
    lp.name::text,
    coalesce(lp.logo_path, lp.logo_url)::text,
    coalesce(pr.clients_count, 0)::integer,
    coalesce(pr.total_points, 0)::numeric,
    coalesce(pr.total_estimated_value, 0)::numeric,
    coalesce(pr.clients, '[]'::jsonb)
  from public.loyalty_programs lp
  left join program_rollup pr on pr.program_id = lp.id
  where lp.active
  order by coalesce(pr.total_points, 0) desc, lp.name asc;
end;
$$;
